...
@section('content')
                 ...
                 <form role="form" action="{{url('/')}}/TypesOfExpenses/{{$TypesOfExpenses->id}}" method="POST" enctype="multipart/form-data">
 					@csrf
 					{{ method_field('PATCH') }}

                   <input type="hidden" name="id" value="{{$TypesOfExpenses->id}}">


            <div class="row">
                <div class="form-group col-md-6">
                               <label class="control-label">{{trans('trans.name_ar')}}</label>
              <input type="text"  value="{{$TypesOfExpenses->name_ar}}" placeholder="{{trans('trans.name_ar')}}" class="form-control"    name="name_ar"  required=""/>
          </div>

          <div class="form-group col-md-6">
            <label class="control-label">{{trans('trans.name_en')}}</label>
<input type="text"  value="{{$TypesOfExpenses->name_en}}" placeholder="{{trans('trans.name_en')}}" class="form-control"    name="name_en"   />
</div>
